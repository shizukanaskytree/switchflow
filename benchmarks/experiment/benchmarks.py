...
class Benchmark():
  def __init__(self,
               ...
               batch_size=32,
               log_steps=10,
               data_format='channels_first', # GPU prefers channels_first.
               skip_eval=True,
               only_inference=False, # Simple inference without frozen steps!
               inter_op_threads=36,
               train_steps=100,
               inference_steps=10, # only 1 step for experiment is bad since cold-start overhead
               create_frozen_model=False, # Prepare frozen model in this file!
               ):
    """
    Args:
  ...
  def get_distribution_stragegy(self):
    return tf.distribute.OneDeviceStrategy("device:GPU:0")

  def get_strategy_scope(self, strategy):
    return strategy.scope()

  def _monkey_patch_dataset_method(self, strategy):
    # The following code is patched into `strategy`'s `make_dataset_iterator`
    # method.
  ...
  def set_up_synthetic_data(self):
    self._monkey_patch_dataset_method(tf.distribute.OneDeviceStrategy)
    self._monkey_patch_dataset_method(tf.distribute.MirroredStrategy)
    if hasattr(tf, 'contrib'):
      self._monkey_patch_dataset_method(tf.contrib.distribute.MirroredStrategy)
      self._monkey_patch_dataset_method(tf.contrib.distribute.OneDeviceStrategy)
    else:
  ...
  def freeze_graph(self, graph, session, output):
    ...
    with graph.as_default():
      ...
      # 1.
      # remove_training_nodes func Explanation:
      ...
      # 1.
      # convert_variables_to_constants func Explanation:
  ...
  def run(self):
    if not self.is_vanilla:
      tf.set_execution_priority(execution_priority=self.priority)
    ...
    # Simple inference without frozen steps
    if self.only_inference:
      ...
      model = self.models[self.model_name](weights='imagenet',
                                           include_top=True, # If True, softmax layer
                                           input_shape=shape)
    else:
      # only do training
    ...
    if self.only_inference:
      inference_input_dataset = None
      inference_input_dataset = input_fn(
        is_training=False,
        data_dir="",  # synthetic dataset has no filepath.
  ...
  def run_frozen_model_inference_helper(self):
    ...
    graph = tf.Graph()
    with graph.as_default():
      net_input, net_out = tf.import_graph_def(
        graph_def,
    ...
    for n in range(num_warmup):
      sess.run(net_out.outputs[0],
               feed_dict={net_input.outputs[0]:inputs})
    print('Warmup Done!')
  ...
  def do_run_inference(self, sess, net_input, net_out, inputs):
    ...
    for i in range(self.inference_steps):
      out = sess.run(net_out.outputs[0],
                     feed_dict={net_input.outputs[0]:inputs})
...
def experiment_training():
  net_list = [
    ...
    'InceptionResNetV2',
    'InceptionV3',
    'MobileNetV2', # only channels_last for prediction!
  ]

  for net in net_list:
    print(net + ' Start! ====================================')
    Benchmark(model_name=net, is_vanilla=True, priority=None,
              inter_op_threads=2, # to tune
              train_steps=500,
...
def experiment_inference():
  ...
  for net in net_list:
    Benchmark(model_name=net, is_vanilla=True, priority=None,
           log_steps=1,
           data_format='channels_first' if net != 'MobileNetV2' else 'channels_last',
           only_inference=True, inference_steps=50,
           ).run()
    print(net + ' Done! ====================================')
